...
var templates: Array[String] = [
	...
	"Ticket clerk at the county bus depot recalls {name} purchasing a one-way fare while carrying pamphlets on {illegal_a}. A torn receipt found in the waste bin references {illegal_b}. The clerk did not record a destination.",
	"Barber shop regulars report {name} steered conversation toward {illegal_a} during a Saturday morning cut. A magazine left on the waiting bench opened to an article about {illegal_b}. The proprietor asked the subject not to return.",
	"Drive-in projectionist notes {name} arrived alone for the second feature and remained through the credits discussing {illegal_a} with neighboring cars. A concession stand napkin bore a sketch labeled {illegal_b}. Gate receipts show no companion ticket was purchased.",
	"County tax office clerk flagged correspondence from {name} citing {illegal_a} as grounds for a property exemption. A follow-up letter questioned assessment practices related to {illegal_b}. Both envelopes were postmarked from the same rural box.",
	"League scorekeeper identifies {name} as a substitute who raised {illegal_a} between frames on Thursday. Printed handouts on {illegal_b} were found taped inside a rental shoe locker. The alley manager requested the subject be barred from future leagues.",
	"Receptionist at Dr. Harlan's office reports {name} left behind reading material on {illegal_a} in the waiting room. A second pamphlet concerning {illegal_b} was tucked inside the magazine rack. Neither item listed a publisher.",
	"Monitoring station logged three broadcasts attributed to a handle linked to {name}. The first transmission referenced {illegal_a}. A second signal on the fourteenth concerned {illegal_b}. A brief third exchange mentioned {illegal_c} before the frequency went silent.",
	"Conductor's report places {name} in compartment seven on the overnight run to Duluth. Fellow passengers overheard remarks about {illegal_a} before dinner. A notebook recovered from the seat pocket contained notes on {illegal_b} and a folded map marked {illegal_c}. The subject disembarked one stop early.",
	"Amateur radio club roster lists {name} as a provisional member who spoke at length about {illegal_a} during the March meeting. Call logs show repeated contact with operators discussing {illegal_b}. A typed bulletin referencing {illegal_c} was circulated after the session. The club secretary resigned the following week.",
	"Municipal parking attendant observed {name} meeting two visitors beside a green sedan. One visitor handed over material related to {illegal_a}. Conversation shifted to {illegal_b} before a third party mentioned {illegal_c} and departed on foot. License plate numbers were not recorded.",
	"Co-op manager reports {name} posted a notice board flyer promoting {illegal_a} near the bulk bins. A second flyer concerning {illegal_b} appeared above the register by Friday. Shoppers reported a verbal pitch on {illegal_c} at the checkout line. All three notices were removed without authorization.",
	"Night clerk at the Starlight Motel registered {name} under a partial alias. Room service notes reference a late delivery of books on {illegal_a}. Housekeeping recovered leaflets about {illegal_b} beneath the mattress. A suitcase lining held correspondence mentioning {illegal_c}. Checkout was paid in cash with no forwarding address.",
]
